...
def compute(df: DataFrame) -> DataFrame:
    ...
    dfgb: DataFrameGroupBy = df.groupby(by=Grouper(key="start", freq="W"))
    ...
    for ts, _df in dfgb:
        ...
        kloc: float = latest_df["lines"].item() / 1000

        data["date"].append(count)
        data["id"].append(latest_df["open_events"].item() / kloc)
...
def plot(df: DataFrame) -> None:
    ...
    # --- BARPLOT ---
    sns.barplot(data=df, x="date", y="id", ax=ax)
    ...
    fig.suptitle("ImageMagick Issue Density per Week", fontsize=12)
    ax.set_xlabel("Week")
