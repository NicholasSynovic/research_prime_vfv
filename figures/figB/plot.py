...
def compute(
    ...
    with Bar("Computing data...", max=commit_pairs.shape[0]) as bar:
        ...
        for idx, commit_pair_row in commit_pairs.iterrows():
            ...
            datum: dict[str, list[Timestamp | float]] = {"date": [], "value": []}
            ...
            for ts, df_group in dfgb:
                ...
                value: float = latest_df["code"].item() / 1000
...
def plot(
    ...
    plt.tight_layout()
    plt.savefig(f"figB_metric{pair_id}.pdf")
    plt.clf()
    plt.close()
...
def main(db_path: Path, csv_path: Path) -> None:
    ...
    id_df: DataFrame = read_bus_factor_from_db(db_engine=db_engine)
