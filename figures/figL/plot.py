# ImageMagick Pull Request Spoilage
...
def compute(df: DataFrame) -> DataFrame:
    ...
    for ts, _df in dfgb:
        ...
        value: int = _df["pr_spoilage"].tail(n=1).item()
...
def compute_cve(
    ...
    for ts, df_group in dfgb:
        ...
        value: int = df_group["pr_spoilage"].tail(n=1).item()
...
def _subplot(ax, df: DataFrame, title: str) -> None:
    ...
    # Plot the legend
    ax.legend(handles=handles, loc="upper right", framealpha=1, frameon=True)
...
def plot(
    ...
    ax1.set_title("ImageMagick Pull Request Spoilage")
    ax1.set_xlabel(xlabel="Year")
    ax1.set_ylabel(ylabel="Pull Request Spoilage")
    ...
    plt.tight_layout()
    plt.savefig(f"figL.pdf")
    plt.clf()
    plt.close()


def main() -> None:
    prs_df: DataFrame = pandas.read_csv(filepath_or_buffer="pr_spoilage.csv")
    prs_df["timestamp"] = prs_df["timestamp"].apply(func=Timestamp)
    prs_df = prs_df.rename(columns={"timestamp": "date"})

    project_metric: DataFrame = compute(df=prs_df)
