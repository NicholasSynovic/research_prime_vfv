...
def compute(df: DataFrame) -> DataFrame:
    data: dict[str, list[Timestamp | int]] = {"date": [], "issue_spoilage": []}
    ...
    for ts, _df in dfgb:
        ...
        data["issue_spoilage"].append(issue_spoilage)
...
def plot(df: DataFrame) -> None:
    ...
    # --- BARPLOT ---
    sns.barplot(data=df, x="date", y="issue_spoilage", ax=ax)
    ...
    fig.suptitle("ImageMagick Issue Spoilage per Week", fontsize=12)
    ax.set_xlabel("Week")
    ax.set_ylabel("Issue Spoilage")
